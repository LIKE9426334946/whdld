...
import torch
from torch.utils.data import DataLoader # 批量读取数据+打乱数据+多线程加载

# 导入自己写的模块
from utils.seed import set_seed
from datasets.transforms import build_transforms # 数据增强
from models.unet_resnet_attn import UNetResNet34Attn # 使用ResNet34
from losses import CombinedLoss
from utils.metrics import ConfusionMatrix # 通过混淆矩阵计算各种指标
from utils.visualize import save_prediction_vis

from utils.split import make_splits, read_split # 等会儿再看
from datasets.whdld import WHDLDDataset, WHDLD_COLORS # 等会儿再看

# 读取yaml配置文件，会返回一个Python字典，注意safe_load()只能使用一次
...
def build_datasets(cfg):
    ...
    img_size = tuple(cfg["data"]["img_size"]) # (256, 256)

    tf_train = build_transforms("train", img_size) # train数据增强
    tf_val = build_transforms("val", img_size) # val数据增强

    if dataset_name == "whdld":
        images_dir = cfg["data"]["images_dir"] # image/Images
        masks_dir  = cfg["data"]["masks_dir"] # imagesPNG/ImagesPNG
        ratio = tuple(cfg["data"]["split_ratio"])
    
        # 写到 runs/splits
        split_dir = os.path.join(cfg["train"]["save_dir"], "splits")
        os.makedirs(split_dir, exist_ok=True) # 不存在就创建，存在不会报错
